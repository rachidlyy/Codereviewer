...
database. Each record carries the public fields the frontend renders, an
`entrypoint` (the function name every language's harness must call - kept
as ONE name shared across languages, so the grader logic never needs to
know which language it's looking at), and a `starter` dict keyed by
language code, so the editor can load the right boilerplate for whichever
language the student has selected.
...
#: knows what code to seed the editor with (per language) and what
...
def to_public(problem: Problem, *, include_entrypoint: bool = False) -> dict[str, Any]:
    ...
    With ``include_entrypoint`` the detail-only fields (starter code per
    language and the function name the runner calls) are added on top of
    the list fields.
    """
