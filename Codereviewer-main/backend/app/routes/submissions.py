...
``POST /api/submissions/run`` executes the submitted code, in the
requested language, against the problem's predefined test cases and
...
def run_code(request: RunRequest) -> dict:
    ...
    return run_submission(problem, request.code, language)
